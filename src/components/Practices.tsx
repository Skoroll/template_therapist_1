    ...
    return(
        <div>
            {myPractices.map((practice) => (
                <div key={practice.id} className="practice-card rounded-2xl flex flex-col bg-mainColor text-white p-3 m-2">
                    <h3  className="text-2xl border-b-2 p-1">{practice.name}</h3>
                    <p className="">{frCards.time} : {practice.time} {frCards.mins}</p>
                    <p>{frCards.price} : {practice.price}€</p>
                    <p>{practice.desc}</p>
